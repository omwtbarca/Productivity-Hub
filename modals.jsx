...
const AddTaskModal = ({ open, onClose, lang, tasks, setTasks, editing }) => {
  ...
  const [dueDays, setDueDays] = useS("0"); // "0" today, "1" tomorrow, "7" week, "none" no due, "custom"
  ...
  const submit = () => {
    ...
    setTasks((tk) => {
      if (isEdit) {
        return {
          ...tk,
          items: tk.items.map(i => i.id === editing.id ? { ...i, text: { en: text, es: text }, groupId, priority, dueISO } : i)
        };
      }
      const newItem = {
        id: "t" + Date.now(),
  ...
  return (
    ...
    >
      ...
      <Field label={tM(lang, "Priority", "Prioridad")}>
        <Radio
